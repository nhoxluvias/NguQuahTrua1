setTimeout(function (e) {
    ...
    var regex = new RegExp("(\/account\/confirm\/)[a-zA-Z0-9-]{1,}(/register-success)$");
    if (regex.test(path)) {
        alert("Đăng ký thành công, hãy nhập mã xác nhận được gửi tới email của bạn để hoàn tất đăng ký");
    }
    regex = new RegExp("(\/account\/confirm\/)[a-zA-Z0-9-]{1,}(/register-success_no-payment-info)$")
    if (regex.test(path)) {
        alert(
    ...
    regex = new RegExp("(\/account\/confirm\/)[a-zA-Z0-9-]{1,}(/login-success_unconfirmed)$");
    if (regex.test(path)) {
        alert(
    ...
    regex = new RegExp("(\/account\/confirm\/)[a-zA-Z0-9-]{1,}(/re-confirm)$")
    if (regex.test(path)) {
        alert(
            "Đã gửi tới email của bạn mã xác nhận mới, vui lòng nhập mã xác nhận để hoàn tất"
        );
    }
    regex = new RegExp("(\/account\/confirm\/)[a-zA-Z0-9-]{1,}(/confirm-failed)$");
    if (regex.test(path)) {
